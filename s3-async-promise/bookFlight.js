...
const delay = (ms) => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        resolve(); // Résout toujours la promesse après le délai
      }, ms);
    }).then(() => {}).catch(() => {});
  };
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        if (flightNumber === "AF123") {
          resolve('le vol est dispo'); // Le vol est disponible la promesse se résout avec true.
        } else {
          reject(new Error(`le vol ${flightNumber} est indisponible`)); // Le vol n'est pas disponible. Sinon, la promesse se rejette avec une erreur.
        }
      }, 2000);
    }).then((value) => {
      console.log(value);
  ...
  const bookFlight = (flightNumber) => {
    ...
    checkFlightAvailability(flightNumber)
      ...
      .catch((error) => {
        ...
        console.error(`Echec de la réservation: ${error.message}`);
      });
  };
